...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 style="font-family: 'Space Grotesk', sans-serif; font-weight: 600;">Edit Pegawai</h2>
    ...
    <div class="form-group">
        ...
        <select name="divisi" 
            ...
            <?php foreach ($divisiList as $divisi): ?>
                <option value="<?= $divisi ?>" <?= old('divisi', $pegawai['divisi']) === $divisi ? 'selected' : '' ?>><?= $divisi ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="form-group">
        ...
        <div class="custom-file">
            <input type="file" 
                   ...
                   accept="image/*">
            <label class="custom-file-label" for="foto_pegawai"><?= !empty($pegawai['foto_pegawai']) ? 'Ganti foto' : 'Pilih file foto' ?></label>
        </div>
...
document.getElementById('foto_pegawai').addEventListener('change', function(e) {
    const fileName = e.target.files[0] ? e.target.files[0].name : '<?= !empty($pegawai['foto_pegawai']) ? 'Ganti foto' : 'Pilih file foto' ?>';
